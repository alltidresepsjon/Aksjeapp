...
function formatNokString(value: string) {
  return new Intl.NumberFormat("nb-NO", { style: "currency", currency: "NOK", minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(
    Number(value)
  );
}
...
export function TradePanel({
  ...
  const canSubmit = !submitting && marketOpen && quantity > 0 && (side === "BUY" || quantity <= ownedQuantity);
  ...
  async function submitOrder() {
    ...
    try {
      const response = await fetch("/api/orders", {
        ...
        body: JSON.stringify({ accountId, ticker, side, quantity, idempotencyKey: generateIdempotencyKey() }),
      });
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="mb-3 flex flex-col gap-1.5">
        ...
        <p className="text-xs text-slate-400">
          {side === "SELL" ? `Du eier ${ownedQuantity} aksjer.` : `Kontantsaldo: ${formatNokString(cashBalance)}`}
        </p>
      </div>

      {estimatedCost && (
        <p className="mb-3 text-sm text-slate-600">
          Anslått {side === "BUY" ? "kostnad" : "salgssum"} før kurtasje/slippage:{" "}
      ...
      {result && (
        <div
          className={`mt-3 rounded-lg px-3 py-3 text-sm ${
            result.status === "FILLED" ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"
          }`}
        >
          {result.status === "FILLED" ? (
            <>
              <p className="font-semibold">
                Ordre utført: {result.side === "BUY" ? "kjøpte" : "solgte"} {result.quantity} {ticker} @{" "}
                {result.fillPrice ? formatNokString(result.fillPrice) : "—"}
              </p>
              <p className="mt-1 text-xs">
                Kurtasje/slippage: {result.feeAmount ? formatNokString(result.feeAmount) : "—"} · Totalt:{" "}
