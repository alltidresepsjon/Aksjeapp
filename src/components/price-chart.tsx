...
export function PriceChart({ points }: { points: PricePoint[] }) {
  if (points.length < 2) {
    return <div className="flex h-32 items-center justify-center text-sm text-slate-400">Ikke nok kursdata ennå.</div>;
  }
  ...
  const areaPath = `${path} L${(padding + (points.length - 1) * xStep).toFixed(1)},${(height - padding).toFixed(1)} L${padding},${(
    height - padding
  ).toFixed(1)} Z`;
